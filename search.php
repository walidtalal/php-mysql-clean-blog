<?php include "includes/header.php";?>
<?php include "config/config.php";?>
<?php
    if(isset($_POST['search'])) {
        if($_POST['search'] =='') {
            header('location: index.php');

        } else {
            $search = $_POST['search'];
            $data = $conn->query("select * from posts where title like '%$search%' and status =1 ");
            $data->execute();
            $rows =$data->fetchAll(PDO::FETCH_OBJ);

            if($data->rowCount() ==0 ) {
                echo "<div class='bg-danger alert alert-danger text-white text-center'>no search for this word</div>";

            }
        }
    } else {
        header('location: index.php');
        
    }
?>


<div class="row gx-4 gx-lg-5 justify-content-center">
    <div class="col-md-10 col-lg-8 col-xl-7">
        <?php if(count($rows)>0): ?>
        <div>Numpor of posts: (<?php echo count($rows)?>)</div>
        <?php //echo 'hello ' . $_SESSION['username'] ?>
        <?php foreach ($rows as $row): ?>
            <!-- Post preview-->
            <div class="post-preview">
                <a href="http://localhost/clean-blog/posts/post.php?post_id=<?php echo $row->id;?>">
                    <h2 class="post-title"><?php echo $row->title; ?></h2>
                    <h3 class="post-subtitle"><?php echo $row->subtitle; ?></h3>
                </a>
                <p class="post-meta">
                    Posted by
                    <a href="#"><?php echo $row->username; ?></a>
                    on <?php echo date('M', strtotime($row->created_at)).','. date('d', strtotime($row->created_at)) ." " .  date('Y', strtotime($row->created_at)) ?>
                </p>
            </div>
            <!-- Divider-->
            <hr class="my-4" />
        <?php endforeach;  ?>

        <?php else:?>
        <div class='bg-danger alert alert-danger text-white text-center'>no search for this word</div>
        <?php endif;?>

        <!-- Pagerr-->
    </div>
</div>

<?php include "includes/footer.php";?>

